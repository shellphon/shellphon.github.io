    ...
    var Game = {
      ...
      start:function(row){
        ...
        var iTmpl = '<div class="item un" {style} data-no="{index}"><div class="back"><span></span></div><div class="front"></div></div>',
            ...
            mapWidth = $('#map').width(),
            itemWidth = 0;
            itemWidth = (mapWidth - row*4)/row;
        for(var i=0; i<Game.num;i++){
            htmlStr+=iTmpl.replace(/{index}/g,i);
        }
        htmlStr = htmlStr.replace(/{style}/g, 'style="width:'+itemWidth+'px;height:'+itemWidth+'px;"');
        $('#map').html(htmlStr);
        datas = dbRandArr(randStr(src,Game.num/2));
        $('#map').unbind().on('tap','.item',function(){
          var that = $(this),
              showing = $('.show'),
              allStep = 0,
              allTime = null,
